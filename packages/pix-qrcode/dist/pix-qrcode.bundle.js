...
class RuleValidator1 {
    ...
    static get(info) {
        const v = new RuleValidator1(info);
        return v;
    }
    addRule(info) {
        return this.addValidator(RuleValidator1.get(info));
    }
    addValidator(rule) {
        rule.parent = this;
    ...
    async executeRule(context) {
        ...
        if (this.ruleInfo.rule) {
            try {
                ...
                result = {
                    status: "fail",
                    error: E instanceof ValidationError1 ? E : new ValidationError1(E)
                };
            }
        }
        return result;
...
class QRCodeError extends ValidationError1 {
    errorCode;
    constructor(errorCode, message){
...
class PIXQRCode {
    ...
    getMAI() {
        const maiList = this.emvQRCode.findIdentifiedTemplate(PIX.GUI, mod.MAI_TEMPLATE_FIRST, mod.MAI_TEMPLATE_LAST);
        return maiList[0];
    }
    ...
    static createCode(elements) {
        const cleanupObject = (obj)=>Object.entries(obj).reduce((a, [k, v])=>v === undefined ? a : (a[k] = v, a), {});
        const pixElements = {
        ...
        const guiNode = new QRCodeNode('data', PIX.GUI, mod.TAG_TEMPLATE_GUI);
        const maiPIX = emvQRCode.newTemplateElement(mod.MAI_TEMPLATE_FIRST, mod.MAI_TEMPLATE_LAST, true, [
            guiNode
        ]);
        if (pixElements.type == "static") {
            if (pixElements.chave) maiPIX.newDataElement(PIX.TAG_MAI_CHAVE, pixElements.chave);
            if (pixElements.infoAdicional) maiPIX.newDataElement(PIX.TAG_MAI_INFO_ADD, pixElements.infoAdicional);
        } else {
            if (pixElements.url) maiPIX.newDataElement(PIX.TAG_MAI_URL, pixElements.url);
        }
        const el62 = emvQRCode.newTemplateElement(mod.TAG_ADDITIONAL_DATA);
        el62.newDataElement(mod.TAG_AD_REF_LABEL, pixElements.txid ?? "***");
        return pixQRCode;
    }
    static parseCode(qrCode, params) {
        params = {
    ...
    extractElements() {
        ...
        if (this.isPIX('static')) {
            return {
                ...
                transactionAmount: emvQR.hasElement(mod.TAG_TRANSACTION_AMOUNT) ? parseFloat(emvQR.getElement(mod.TAG_TRANSACTION_AMOUNT).content) : undefined,
                txid: emvQR.getElement(mod.TAG_ADDITIONAL_DATA)?.getElement(mod.TAG_AD_REF_LABEL)?.content
            };
...
export { PIXPayloadRetriever as PIXPayloadRetriever };
var PIXPayload;
(function(PIXPayload) {
    ...
    PIXPayload.fromJSON_v2 = fromJSON_v2;
    ...
    function validatePayload(_payload, options = {
        isCobV: false
    }) {}
    PIXPayload.validatePayload = validatePayload;
})(PIXPayload || (PIXPayload = {}));
export { PIXPayload as PIXPayload };
export { PIXQRCodeError as PIXQRCodeError, PIXQRErrorCode as PIXQRErrorCode };
